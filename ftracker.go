...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	heightInM := height / (float64(cmInM))
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeed, float64(2))/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * float64(minInH))
}
