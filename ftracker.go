...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeed, float64(2))/(height/(float64(cmInM))))*walkingSpeedHeightMultiplier*weight) * duration * float64(minInH))
}
